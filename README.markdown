...
- In Paradoc, `#` no longer performs exponentiation; that is relegated to the (non-ASCII character) `ˆ` or `*p`, which seems a more reasonable mnemonic anyway (although I liked the audial mnemonic of **Pound** and **Power**) and allows it to also perform list exponentiation in the sense of Cartesian products.
